...
Page({
  ...
  deleteCategory(data){
    ...
    collectremoveCollectCategory(data.detail.params).then(res=>{
  ...
  selectButtomVarietiesFn(varietyId) {
    ...
    console.log(this.data.varietyId,'varietyId')
    selectButtomVarieties({}).then((res) => {
      this.setData({
        varieties: res.filter(v => v.varietyId == this.data.varietyId),
        "pricingDetail.varietyId":this.data.varietyId
      })
