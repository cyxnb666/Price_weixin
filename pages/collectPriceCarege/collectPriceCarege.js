import {
  ...
  selectVarietySpecss,
  filepreview,
  saveCollectCollectCategoryPrice,
  geCollectCollectCategory
} from "../../utils/api";
import {
  Toast
...
Page({
  ...
  loadExistingData() {
    ...
    geCollectCollectCategory(params).then((res) => {
      ...
      this.setData({
        smallVarieties:res.smallVarieties,
        collectPriceId:res.collectPriceId
      })
  ...
  saveData(submitType) {
    ...
    const params = {
      condition: {
        ...
        smallVarieties:this.data.smallVarieties,
        submitType: submitType
      }
    };

    console.log('保存数据参数:', params);
    saveCollectCollectCategoryPrice(params).then((res) => {
  ...
  filterValidData(dataArray) {
    return dataArray.filter(item =>
      item.saleChannelCode && item.specsId && item.unitPrice);
  },
