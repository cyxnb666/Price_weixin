...
Page({
  ...
  onLoad: function (options) {
    ...
    let uploadParamskey = wx.getStorageSync('uploadParamskey')
    console.log(uploadParamskey, 'uploadParamskey')
    if (uploadParamskey) {
      this.setData({
        varietyId: uploadParamskey.varietyId,
        collectPriceId: uploadParamskey.taskId,
        uploadType: uploadParamskey.type
      })
      this.getSmallSelectCategories(uploadParamskey.type)
    }
    // 设置屏幕常亮
    wx.setKeepScreenOn({
      keepScreenOn: true,
  ...
  uploadFile(tempFilePaths, key) {
    ...
    console.log({
      ...
      collectType: obj[this.data.uploadType],
      collectCategoryId: this.data.selectedType,
      collectPriceId: this.data.collectPriceId,
    }, '上传参数')

    const that = this
    wx.uploadFile({
      url: `${env.baseURL}/file/uploadCollectCategoryFileWithWater`, //仅为示例，非真实的接口地址
      ...
      success(res) {
        ...
        if (retCode === 200) {
          ...
          // })
          that.getSmallSelectCategories(that.data.uploadType)
