import {
  ...
  filepreview,
  softRemoveFile,
  savemarketCollectCategoryPrice,
  marketSelectCategories,
  getFruitMarketCategory
} from "../../utils/api";
import {
  Toast
...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({
      fruitMarketId: options.fruitmarketid,
  ...
  loadExistingData() {
    ...
    getFruitMarketCategory(params).then((res) => {
      ...
      if (res.priceFiles && Array.isArray(res.priceFiles)) {
        if (res.priceFiles.length > 0 && typeof res.priceFiles[0] === 'object') {
          priceFileIds = res.priceFiles.map(file => file.fileId);
        } else {
          priceFileIds = res.priceFiles;
        }
      }
  ...
  saveData(submitType) {
    ...
    savemarketCollectCategoryPrice(params).then((res) => {
