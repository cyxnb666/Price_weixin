...
import {
  ...
  queryTypeDicts,
  regeo,
  largesaleRemoveCollectCategory,
  filepreview,
  reportLargePrice,
...
Component({
  ...
  ready() {
    ...
    console.log(this.data.salePlanDetail,'salePlanDetail')
    this.setData({
      delta: this.data.tallInfo.delta,
      // categoryIds:this.data.tallInfo.categoryId.join(','),
      stallId: this.data.tallInfo.stallId,
      collectPriceId: this.data.tallInfo.collectPriceId == "null" ? '' :this.data.tallInfo.collectPriceId ,
  ...
  methods: {
    ...
    deleteCategory(data){
      ...
      largesaleRemoveCollectCategory(data.detail.params).then(res=>{
